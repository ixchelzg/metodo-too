...
from management.models import EquipoDeComputo, Tipo, Estado
...
class BreakdownPieDrawing(_DrawingEditorMixin,Drawing):
    def __init__(self,width=600,height=400,*args,**kw):
        ...
        for es in Estado.objects.all():
            valor=EquipoDeComputo.objects.filter(estado=es.id).count()
            valores.append(valor)
            leyendas.append(es.nombre)

        self.pie.data = valores
        self.pie.labels = leyendas
